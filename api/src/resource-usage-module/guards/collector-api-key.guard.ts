...
export class CollectorApiKeyGuard implements CanActivate {
	...
	canActivate(context: ExecutionContext): boolean {
		...
		const validApiKey = this.configService.get<string>('COLLECTOR_API_KEY')?.trim();
	...
	private normalizeHeaderValue(headerValue: string | string[] | undefined): string | undefined {
		if (!headerValue) {
			return undefined;
		}
